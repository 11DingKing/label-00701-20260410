...
const onFilesAdded = (tracks) => {
  ...
  console.log(
    `[App] 接收到 ${tracks.length} 个音频文件, 当前列表: ${playlist.value.length} 首`,
  );
...
const prevTrack = () => {
  ...
  console.log(
    `[App] 上一曲: [${oldIndex}] -> [${currentIndex.value}] ${playlist.value[currentIndex.value].name}`,
  );
  isPlaying.value = true;
};
...
const nextTrack = () => {
  ...
  if (isLoop.value) {
    ...
    // 触发重新播放：先暂停再播放，或者通过其他方式
    // 这里我们可以通过切换播放状态来触发重新加载
    isPlaying.value = false;
    setTimeout(() => {
      isPlaying.value = true;
    }, 0);
  ...
  console.log(
    `[App] 下一曲: [${oldIndex}] -> [${currentIndex.value}] ${playlist.value[currentIndex.value].name}`,
  );
  isPlaying.value = true;
};
...
// 循环状态变化
const onLoopChange = (loopState) => {
  isLoop.value = loopState;
  console.log(`[App] 循环模式切换: ${loopState ? "开启" : "关闭"}`);
};

...
const loadTrack = (track, shouldPlay = false) => {
  ...
  console.log(
    `[PlayerControls] 加载曲目: ${track.name}, URL: ${track.url?.substring(0, 50)}...`,
  );
...
watch(
  ...
  (newTrack, oldTrack) => {
    if (newTrack && newTrack.id !== currentTrackId) {
      console.log(
        `[PlayerControls] 曲目变化: ${oldTrack?.name || "无"} -> ${newTrack.name}`,
      );
...
watch(
  ...
  (playing) => {
    ...
    if (playing) {
      console.log(
        `[PlayerControls] 开始播放: ${props.currentTrack?.name || "未知"}`,
      );
      audio.value.play().catch((err) => {
...
onMounted(() => {
  nextTick(() => {
    if (props.currentTrack && audio.value) {
      console.log(
        `[PlayerControls] 组件挂载, 初始化曲目: ${props.currentTrack.name}`,
      );
...
const updateProgress = () => {
  ...
  if (switchMode.value === "time" && !isLoop.value && props.isPlaying) {
    const acc =
      accumulatedTime +
      (lastUpdateTime > 0 ? (Date.now() - lastUpdateTime) / 1000 : 0);
    if (acc >= fixedDuration.value) {
      console.log(
        `[PlayerControls] 定时切换触发: 累计播放 ${acc.toFixed(1)}s >= ${fixedDuration.value}s`,
      );
      accumulatedTime = 0;
      lastUpdateTime = Date.now();
      emit("next-track");
...
const handleEnded = () => {
  console.log(
    `[PlayerControls] 曲目播放结束: ${props.currentTrack?.name || "未知"}, 循环模式: ${isLoop.value}`,
  );
  if (isLoop.value) {
...
const handleError = (e) => {
  ...
  console.error(
    `[PlayerControls] 音频播放错误: ${props.currentTrack?.name || "未知"}, 错误码: ${errorCode}, 错误信息: ${errorMsg}`,
    e,
  );
  emit("toast", "音频加载失败");
};
...
const startSeek = (e) => {
  ...
  const onEnd = () => {
    ...
    console.log(
      `[PlayerControls] 进度拖拽完成: ${seekVal.toFixed(1)}s, 累计播放时长保持不变`,
    );
...
const toggleLoop = () => {
  ...
  console.log(
    `[PlayerControls] 循环模式切换: ${isLoop.value ? "开启" : "关闭"}`,
  );
  emit("toggle-loop", isLoop.value);
...
const onModeChange = () => {
  ...
  console.log(
    `[PlayerControls] 切换模式变更: ${switchMode.value}, 定时时长: ${fixedDuration.value}s`,
  );
  emit(
    "toast",
    switchMode.value === "end"
      ? "播放完毕后切换"
      : "每 " + fixedDuration.value + " 秒切换",
  );
};
</script>
